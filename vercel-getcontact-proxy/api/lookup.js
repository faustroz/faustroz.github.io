...
const UPSTREAM_PATH = "/cgi-bin/lookup.py";
...
async function lookup(request, response) {
  ...
  try {
    ...
    if (upstream.status >= 400) console.warn("getcontact proxy upstream error", { status: upstream.status });
    response.status(upstream.status);
    response.setHeader("Content-Type", upstream.contentType);
    return response.send(upstream.body);
  } catch (error) {
    console.error("getcontact proxy upstream network error", { message: error instanceof Error ? error.message : "Unknown network error" });
