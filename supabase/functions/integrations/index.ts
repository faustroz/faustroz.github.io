...
// Secrets are read only here: supabase secrets set GITHUB_TOKEN=... GOOGLE_CLIENT_ID=... GOOGLE_CLIENT_SECRET=... AI_API_KEY=...
...
Deno.serve(async (request) => {
  ...
  if (!["github", "google_calendar", "ai"].includes(provider)) return json({ error: "Unsupported provider" }, 400);
  ...
  const configured = Boolean(provider === "github" ? Deno.env.get("GITHUB_TOKEN") : provider === "google_calendar" ? Deno.env.get("GOOGLE_CLIENT_ID") && Deno.env.get("GOOGLE_CLIENT_SECRET") : Deno.env.get("AI_API_KEY"));
