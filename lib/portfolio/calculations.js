...
export function calcAssetPosition(transactions) {
  const isManualValueAsset = ['Reksa Dana', 'Deposito'].includes(transactions[0]?.category);

  if (isManualValueAsset) {
    let totalCost = 0;
...
export function calcPortfolioSummary(transactions, prices) {
  ...
  for (const [ticker, asset] of Object.entries(assetMap)) {
    ...
    const isManualValueAsset = ['Reksa Dana', 'Deposito'].includes(asset.category);
    const currentPrice = isManualValueAsset ? position.avgBuyPrice : (priceData?.priceIDR ?? 0);
