...
const TABLE_QUERIES = Object.freeze([
  ...
  ["study_exams", "id,title,exam_date,score,target_score,updated_at"],
  ["study_flashcards", "id,front,topic,mastery,updated_at"],
  ...
  ["ai_memory_entries", "id,title,kind,importance,updated_at"],
]);
...
export function deriveOperationsSnapshot(records) {
  ...
  const exams = records.study_exams || [];
  const flashcards = records.study_flashcards || [];
  const projects = records.hub_projects || [];
  const tasks = records.project_tasks || [];
  const changelog = records.project_changelog || [];
  const memory = records.ai_memory_entries || [];
  ...
  const recentActivity = [
    ...
    ...exams.map((record) => recordActivity(record, "Study", `Exam: ${record.title}`)),
    ...
    ...memory.map((record) => recordActivity(record, "AI Memory", `Memory: ${record.title}`)),
  ]
  ...
  return {
    ...
    memory: {
      entryCount: memory.length,
      byKind: countBy(memory, "kind"),
    },
    currentFocus,
    recentActivity,
    empty: {
      finance: expenses.length === 0 && subscriptions.length === 0 && budgets.length === 0,
      study: topics.length === 0 && exams.length === 0 && flashcards.length === 0,
