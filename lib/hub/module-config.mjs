export const FINANCE_CHANNELS = Object.freeze([
  ...
  {
    id: "transfers", label: "Transfers", table: "bank_transfers", title: "Account transfers", orderBy: "transferred_on",
    description: "Move money between your own accounts without counting it as income or expense.",
    fields: [
      { name: "from_account_id", label: "Transfer from", type: "lookup", lookup: { table: "bank_accounts", value: "id", label: ["bank_name", "name"] }, displayLookupLabel: true, required: true },
      { name: "to_account_id", label: "Transfer to", type: "lookup", lookup: { table: "bank_accounts", value: "id", label: ["bank_name", "name"] }, displayLookupLabel: true, required: true },
      { name: "amount", label: "Amount", type: "number", format: "currency", min: 0.01, step: 0.01, required: true },
      { name: "transferred_on", label: "Transfer date", type: "date", required: true },
      { name: "notes", label: "Notes", type: "textarea", defaultValue: "" },
    ],
  },
  {
    id: "reconciliation", label: "Reconciliation", table: "bank_account_reconciliations", title: "Bank reconciliation", orderBy: "reconciled_on",
    description: "Compare the balance recorded in 4allx with the balance shown by your bank or cash account.",
    fields: [
      { name: "bank_account_id", label: "Bank / account", type: "lookup", lookup: { table: "bank_accounts", value: "id", label: ["bank_name", "name"] }, displayLookupLabel: true, required: true },
      { name: "statement_balance", label: "Actual bank balance", type: "number", format: "currency", step: 0.01, required: true },
      { name: "ledger_balance", label: "4allx balance snapshot", type: "computed", format: "currency" },
      { name: "difference", label: "Difference", type: "computed", format: "currency" },
      { name: "reconciled_on", label: "Checked on", type: "date", required: true },
      { name: "notes", label: "Notes", type: "textarea", defaultValue: "" },
    ],
  },
  {
  ...
  {
    id: "categories", label: "Categories", sections: [
      {
        id: "expense-categories", table: "finance_categories", title: "Expense Categories", orderBy: "updated_at",
        description: "Create and manage labels used only by expense entries and category budgets.",
        recordScope: { kind: "expense" }, fixedValues: { kind: "expense" },
        fields: [{ name: "name", label: "Category name", required: true }, { name: "color", label: "Label color", type: "color", defaultValue: "#a1a1aa", required: true }],
      },
      {
        id: "income-categories", table: "finance_categories", title: "Income Categories", orderBy: "updated_at",
        description: "Create and manage labels used only by income entries.",
        recordScope: { kind: "income" }, fixedValues: { kind: "income" },
        fields: [{ name: "name", label: "Category name", required: true }, { name: "color", label: "Label color", type: "color", defaultValue: "#a1a1aa", required: true }],
      },
    ],
  },
...
export const ACADEMIC_CHANNELS = Object.freeze([
  ...
  { id: "mku", label: "Mata Kuliah Umum", table: "academic_mku_records", title: "Mata Kuliah Umum", orderBy: "updated_at", filter: { field: "semester", label: "SEMESTER" }, description: "Record general-course grades separately from medical block assessments.", displayFields: ["course_name", "semester"], fields: [
    { name: "course_name", label: "Course name / Nama mata kuliah", required: true }, { name: "credits", label: "SKS", type: "number", min: 0.5, step: 0.5, inputMode: "decimal", placeholder: "e.g. 2", required: true }, { name: "grade", label: "Grade", type: "select", options: ["A", "A-", "B+", "B", "B-", "C+", "C", "D", "E"], defaultValue: "A" }, { name: "semester", label: "Semester", required: true },
  ] },
]);
