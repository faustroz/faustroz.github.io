export const FINANCE_CHANNELS = Object.freeze([
  ...
  ] },
  {
    ...
    fields: [
      ...
      { name: "bank_account_name", label: "Received to (bank/account)", type: "lookup", lookup: { table: "bank_accounts", value: "name", label: ["name", "bank_name", "account_type"] }, defaultValue: "", optional: true },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: "bank_account_name", label: "Paid from (bank/account)", type: "lookup", lookup: { table: "bank_accounts", value: "name", label: ["name", "bank_name", "account_type"] }, defaultValue: "", optional: true },
...
export const ACADEMIC_CHANNELS = Object.freeze([
  { id: "records", label: "Academic record", table: "academic_records", title: "Grades & credits", orderBy: "updated_at", description: "Record grade, credits/SKS, semester, and block. IP/IPK is calculated from these private records.", fields: [
    { name: "course_name", label: "Course", required: true }, { name: "credits", label: "SKS", type: "number", min: 0.5, step: 0.5, inputMode: "decimal", placeholder: "e.g. 4", required: true }, { name: "grade", label: "Grade", type: "select", options: ["A","A-","B+","B","B-","C+","C","D","E"], defaultValue: "A" }, { name: "semester", label: "Semester", required: true }, { name: "block", label: "Block", defaultValue: "" },
  ] },
]);
