...
test("Phase 12 exposes academic records and real financial goals", () => { assert.equal(ACADEMIC_CHANNELS[0].table, "academic_records"); assert.equal(FINANCE_CHANNELS.find(({ id }) => id === "goals").table, "financial_goals"); assert.ok(SEARCH_SOURCES.some(({ table }) => table === "academic_records")); });
