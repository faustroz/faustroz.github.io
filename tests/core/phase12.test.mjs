...
test("Academic records support semester filtering", () => { assert.equal(ACADEMIC_CHANNELS[0].table, "academic_records"); assert.equal(ACADEMIC_CHANNELS[0].filter.field, "semester"); assert.equal(ACADEMIC_CHANNELS[0].fields.find(({ name }) => name === "credits").step, 0.5); assert.equal(FINANCE_CHANNELS.find(({ id }) => id === "goals").table, "financial_goals"); assert.ok(SEARCH_SOURCES.some(({ table }) => table === "academic_records")); });
